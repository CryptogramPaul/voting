...
<div class="row">
    <div class="col-12 col-xl-6 ">
        <div class="card p-4">
            ...
            <?php
                // OVERALL PERCENTAGE
                $sql_total_students = $conn->prepare("SELECT COUNT(*) FROM tb_students WHERE schoolyear = ? ");
            ...
            <!-- <?php
                ...
                $sql_beed = $conn->prepare("SELECT count(*) as vote
                                                FROM tb_vote a 
                                                LEFT JOIN tb_students b ON a.studid = b.studid 
                                                WHERE b.course = 'BEED' AND a.schoolyear = ? GROUP BY b.course ");
                $sql_beed->execute([$schoolyear]);
                $beed_count = $sql_beed->fetchColumn(0); 

                $sql_total_students = $conn->prepare("SELECT COUNT(*) FROM tb_students WHERE course = 'BEED' AND schoolyear = ? ");
                $sql_total_students->execute([$schoolyear]);
                $total_students = $sql_total_students->fetchColumn();
